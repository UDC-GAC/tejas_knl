...
public class Directory extends Cache implements Coherence {
    ...
    public void writeHit(long addr, Cache c) {
        // XXX
        // if(ArchitecturalComponent.getCore(0).getNoOfInstructionsExecuted()>3000000l)
        // {
        // System.out.println("Directory WriteHit t : " +
        // GlobalClock.getCurrentTime() + " addr : " + addr + " cache : " + c);
        // }
        writeHitAccesses++;
    ...
    public void readMiss(long addr, Cache c) {
        ...
        // if(ArchitecturalComponent.getCore(0).getNoOfInstructionsExecuted()>3000000l)
        // {
        // System.out.println("Directory ReadMiss t : " +
        // GlobalClock.getCurrentTime() + " addr : " + addr + " cache : " + c);
        // }
        readMissAccesses++;
    ...
    public void writeMiss(long addr, Cache c) {
        ...
        // if(ArchitecturalComponent.getCore(0).getNoOfInstructionsExecuted()>3000000l)
        // {
        // System.out.println("Directory WriteMiss t : " +
        // GlobalClock.getCurrentTime() + " addr : " + addr + " cache : " + c);
        // }
        writeMissAccesses++;
        sendAnEventFromCacheToDirectory(addr, c,
                RequestType.DirectoryWriteMiss);
    }
    
    private AddressCarryingEvent sendAnEventFromCacheToDirectory(long addr,
            Cache c, RequestType request) {
    ...
    public AddressCarryingEvent evictedFromSharedCache(long addr, Cache c) {
        ...
        // if(ArchitecturalComponent.getCore(0).getNoOfInstructionsExecuted()>3000000l)
        // {
        // System.out.println("Directory EvictShared t : " +
        // GlobalClock.getCurrentTime() + " addr : " + addr + " cache : " + c);
        // }
        evictedFromSharedCacheAccesses++;
        return sendAnEventFromCacheToDirectory(addr, c,
                RequestType.DirectoryEvictedFromSharedCache);
    }
    ...
    public AddressCarryingEvent evictedFromCoherentCache(long addr, Cache c) {
        ...
        // if(ArchitecturalComponent.getCore(0).getNoOfInstructionsExecuted()>3000000l)
        // {
        // System.out.println("Directory EvictCoherent t : " +
        // GlobalClock.getCurrentTime() + " addr : " + addr + " cache : " + c);
        // }
        evictedFromCoherentCacheAccesses++;
        return sendAnEventFromCacheToDirectory(addr, c,
                RequestType.DirectoryEvictedFromCoherentCache);
    }
    ...
    public void handleEvent(EventQueue eventQ, Event e) {
        ...
        // if(ArchitecturalComponent.getCores()[0].getNoOfInstructionsExecuted()
        // > 4000000) {
        // System.out.println("\n\nDirectory handleEvent currEvent : " + event);
        // toStringPendingEvents();
        // }
        
        if (access(addr) == null && (reqType == RequestType.DirectoryWriteHit
        ...
        switch (dirEntry.getState()) {
            ...
            case SHARED: {
                ...
                if (dirEntry.isSharer(c) == true) {
                    ...
                    // write entry in c1's mshr entry for addr
                    // c1's writeHit reaches directory. The directory
                    // re-configured the owner to c1
                ...
                dirEntry.setState(MESIF.SHARED);
                dirEntry.addSharer(c);
            ...
            case INVALID: {
                ...
                // is just a minor hack to maintain readability of the code
    ...
    private void sendCachelineForwardRequest(Cache ownerCache,
            Cache destinationCache, long addr) {
    ...
    public void printStatistics(FileWriter outputFileWriter)
        ...
        outputFileWriter.write("Directory Access due to ReadMiss\t=\t"
                + readMissAccesses + "\n");
        outputFileWriter.write("Directory Access due to WriteMiss\t=\t"
                + writeMissAccesses + "\n");
        outputFileWriter.write("Directory Access due to WriteHit\t=\t"
                + writeHitAccesses + "\n");
        outputFileWriter
                .write("Directory Access due to EvictionFromCoherentCache\t=\t"
                        + evictedFromCoherentCacheAccesses + "\n");
        outputFileWriter
                .write("Directory Access due to EvictionFromSharedCache\t=\t"
                        + evictedFromSharedCacheAccesses + "\n");
        
        outputFileWriter.write("Directory Hits\t=\t" + hits + "\n");
        outputFileWriter.write("Directory Misses\t=\t" + misses + "\n");
        if ((hits + misses) != 0) {
            outputFileWriter.write("Directory Hit-Rate\t=\t"
                    + Statistics.formatDouble((double) (hits) / (hits + misses))
                    + "\n");
            outputFileWriter
                    .write("Directory Miss-Rate\t=\t"
