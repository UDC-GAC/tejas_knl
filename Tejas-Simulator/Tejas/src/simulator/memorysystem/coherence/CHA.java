...
//		}
    ...
    public CHA getDestinationCHA(long addr) {
	long q = addr % 8L;
	int quadrant = 0;
	switch ((int)q) {
	    case 0:
	    ...
	    case 4:
	    case 5: quadrant = 2; break;
	    case 6:
   	    case 7: quadrant = 3; break;
       	    default: break;
	}
	int offset = quadrant * 9;
	long index = addr % 9L;
	CHA c = (CHA)SystemConfig.chaList.get((int)index + offset);
	return c;
    }
	...
	private AddressCarryingEvent sendAnEventFromCacheToDirectory(long addr, Cache c, RequestType request) {
		...
		// Create an event
		CHA directory = this;
		if (addr>=SystemConfig.mcdramAddr) {
		    directory = getDestinationCHA(addr);
		}
	...
	private void forceInvalidate(CacheLine dirEntry) {
		...
		for (Cache sharerCache : dirEntry.getSharers()) {
			sharerCache.updateStateOfCacheLine(dirEntry.getAddress(), MESIF.INVALID);
		}
...
//				System.out.println("Evicted line : " + (evictedEntry.getAddress()>>blockSizeBits) + "\n" + evictedEntry);
	...
	public void printStatistics(FileWriter outputFileWriter) throws IOException {
		...
		outputFileWriter.write("CHA[" + id + "] Access due to ReadMiss\t=\t" + readMissAccesses + "\n");
		outputFileWriter.write("CHA[" + id + "] due to WriteMiss\t=\t" + writeMissAccesses + "\n");
		outputFileWriter.write("CHA[" + id + "] due to WriteHit\t=\t" + writeHitAccesses + "\n");
		outputFileWriter.write("CHA[" + id + "] due to EvictionFromCoherentCache\t=\t" + evictedFromCoherentCacheAccesses + "\n");
		outputFileWriter.write("CHA[" + id + "] due to EvictionFromSharedCache\t=\t" + evictedFromSharedCacheAccesses + "\n");
		
		outputFileWriter.write("CHA[" + id + "] Hits\t=\t" + hits + "\n");
		outputFileWriter.write("CHA[" + id + "]\t=\t" + misses + "\n");
		if ((hits+misses) != 0) {
			outputFileWriter.write("CHA[" + id + "] Hit-Rate\t=\t" + Statistics.formatDouble((double)(hits)/(hits+misses)) + "\n");
			outputFileWriter.write("CHA[" + id + "] Miss-Rate\t=\t" + Statistics.formatDouble((double)(misses)/(hits+misses)) + "\n");
		}
	}
