...
public class Cache extends SimulationElement {
	...
	public void sendRequestToNextLevel(long addr, RequestType requestType) {
		...
		//if ((addr>=SystemConfig.mcdramAddr)&&(SystemConfig.mcdramAddr!=-1)&&(c==null))
		//    System.out.println("sendRequestToNextLevel(" + addr + ")");
		if (c != null) {
...
//				NucaCache nuca = ArchitecturalComponent.nucaList.get(nextLevelName);
			...
			if ((SystemConfig.mcdramAddr==-1)||(SystemConfig.ddrAddr==-1)) {
				checkAddr();
			} else {
			    checked = true;
			}
			
			if((SystemConfig.knl==true)&&(checked == true)&&
			   (((SystemConfig.ddrAddr<=addr)&&(addr<SystemConfig.ddrAddr+SystemConfig.ddrSize))||
			    (SystemConfig.mcdramAddr<=addr)&&(SystemConfig.mcdramAddr + SystemConfig.mcdramSize>addr))) {
			    //System.out.println("access: " + addr);
				memController = getComInterface()
						.chooseMCDRAMorDDRmodule(addr);
			}
			else if(SystemConfig.memControllerToUse==true){
			    ...
			    memController = getComInterface()
				    .getNearestMemoryController((int)0);	
			}
	    ...
	    try { 
	        br = new BufferedReader(new FileReader(SystemConfig.addrFilePath));
		...
		while (line != null) {
			...
			SystemConfig.setMCDRAMsize(size);
			SystemConfig.setMCDRAMaddr(addr);
		    }
		    if (module.equals("DDR")) {
			SystemConfig.setDDRsize(size);
			SystemConfig.setDDRaddr(addr);
		    }
		    line = br.readLine();
		}
	    } catch (Exception e) {
