...
public class NOC extends InterConnect {
	...
	public MainMemoryDRAMController getNearestMemoryController(ID currBankId) {
		...
		//System.out.println("addr: [" + addr + "]\tDRAM(" + SystemConfig.ddrAddr + ")\tMC(" + SystemConfig.mcdramAddr + ")");
		if ((addr>= SystemConfig.ddrAddr)&&
		    (addr < SystemConfig.ddrAddr + SystemConfig.ddrSize)) {
		    index = (addr >>> 3) % 2L;
		    memControllerRet = ArchitecturalComponent.memoryControllers.get((int)index);
		    //System.out.println("DDR[" + index + "]");
		} else if((addr>= SystemConfig.mcdramAddr)&&
			  (addr < SystemConfig.mcdramAddr + SystemConfig.mcdramSize)) {
		    index = (addr >>> 6) % 8L;
		    memControllerRet = ArchitecturalComponent.mcdramControllers.get((int)index);
		    //System.out.println("MCDRAM[" + index + "]:\t" + addr + "\t" + (addr>>>3));
		}
