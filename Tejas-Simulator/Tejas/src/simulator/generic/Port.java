...
public class Port {
    ...
    public Port(PortType portType, int noOfPorts, long occupancy,
        ...
        else if (portType != PortType.Unlimited && noOfPorts > 0
                && occupancy > 0) {
    ...
    public void put(Event event, int noOfSlots) {
        if (this.portType == PortType.Unlimited) {
            ...
            event.addEventTime(
                    GlobalClock.getCurrentTime() + latencyOfConnectedElement);
            ...
            for (int i = 0; i < noOfPorts; i++) {
                if (portBusyUntil[i] < portBusyUntil[availablePortID]) {
                    availablePortID = i;
                }
            }
            ...
            // current time
            if (portBusyUntil[availablePortID] < GlobalClock.getCurrentTime()) {
            ...
            event.addEventTime(
                    portBusyUntil[availablePortID] + latencyOfConnectedElement);
