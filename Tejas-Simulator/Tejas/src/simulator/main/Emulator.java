...
public class Emulator {
    ...
    public Emulator(String pinTool, String pinInstrumentor,
        ...
        System.out.println("subset sim size = " + SimulationConfig.subsetSimSize
                + "\t" + SimulationConfig.subsetSimulation);
        
        System.out.println("marker functions = "
        ...
        if (OS.indexOf("win") >= 0) {
            cmd = new StringBuilder(pin + // " -injection child "+
                    " -t " + pinInstrumentor + " -maxNumActiveThreads "
                    + (SystemConfig.maxNumJavaThreads
                            * SystemConfig.numEmuThreadsPerJavaThread)
                    + " -map " + SimulationConfig.MapEmuCores + " -numIgn "
                    + SimulationConfig.NumInsToIgnore + " -numSim "
                    + SimulationConfig.subsetSimSize + " -id " + pid
                    + " -traceMethod "
                    + EmulatorConfig.communicationType.toString());
        } else {
            String affinity = "scatter ";
            if (SystemConfig.coreAffinityMode==SystemConfig.Affinity.Colocated) {
                affinity = "proclist=[0,1,20,21,28,29,50,51,56,57,36,37,44,45,62,63,48,49,24,25,4,5,10,11,16,17,40,41,32,33,58,59,18,19,42,43,60,61,54,55,34,35,12,13,6,7,26,27,46,47,14,15,8,9,38,39,30,31,2,3,22,23,52,53],explicit ";
            }
            cmd = new StringBuilder(
                    "sudo OMP_NUM_THREADS=64 KMP_AFFINITY=" + affinity
                            + pin + " -ifeellucky -injection child " + " -t "
                            + pinInstrumentor + " -maxNumActiveThreads "
                            + (SystemConfig.maxNumJavaThreads
                                    * SystemConfig.numEmuThreadsPerJavaThread)
    ...
    public Emulator(String pinTool, String pinInstrumentor,
        ...
        StringBuilder cmd = new StringBuilder(pin + // " -injection child "+
                " -t " + pinInstrumentor + " -maxNumActiveThreads  "
                + EmulatorConfig.maxThreadsForTraceCollection + " -map "
                + SimulationConfig.MapEmuCores + " -numIgn "
                + SimulationConfig.NumInsToIgnore + " -numSim "
        ...
        if (SimulationConfig.pinpointsSimulation == true) {
            misc.Error.showErrorAndExit(
                    "Cannot create a trace file, and a pinpoints file at the same time !!");
        }
        if (SimulationConfig.startMarker != "") {
            cmd.append(" -startMarker " + SimulationConfig.startMarker);
        }
    ...
    private void startEmulator(String cmd) {
        ...
        try {
            ...
            if (isStreamGobblerNeeded == true) {
                s1 = new TejasStreamGobbler("stdin",
                        emulatorProcess.getInputStream());
                s2 = new TejasStreamGobbler("stderr",
                        emulatorProcess.getErrorStream());
    ...
    // Should wait for PIN too before calling the finish function to deallocate
    // stuff related to
    ...
    public void forceKill() {
        ...
        if (EmulatorConfig.emulatorType == EmulatorType.pin) {
            ...
            Process process;
            if (OS.indexOf("win") >= 0) {
                String cmd = "cmd.exe /c " + EmulatorConfig.KillEmulatorScript
                        + " " + String.valueOf(Main.pid);
                try {
                    process = Runtime.getRuntime().exec(cmd);
                    TejasStreamGobbler s1 = new TejasStreamGobbler("stdin",
                            process.getInputStream());
                    TejasStreamGobbler s2 = new TejasStreamGobbler("stderr",
                ...
                String cmd[] = { "/bin/bash", EmulatorConfig.KillEmulatorScript,
                        String.valueOf(Main.pid) };
                try {
                    process = Runtime.getRuntime().exec(cmd);
                    TejasStreamGobbler s1 = new TejasStreamGobbler("stdin",
